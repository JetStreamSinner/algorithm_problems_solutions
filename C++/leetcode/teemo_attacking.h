...
// inclusive time interval [t, t + duration - 1]. If Teemo attacks again before the poison effect ends,
...
class Solution {
public:
    int32_t findPoisonedDuration(std::vector<int32_t>& timeSeries, int32_t duration) {
        int32_t total_duration = duration;
        auto previous = timeSeries.begin();
        auto forwarder = std::next(previous);
        const auto end = timeSeries.end();

        while (forwarder < end) {
            const int32_t previous_end_at = *previous + duration;
            if (*forwarder - *previous > duration) {
                total_duration += duration;
            } else {
                total_duration += *forwarder - *previous;
            }
